...
function StudentPaymentsPage() {
  ...
  useEffect(() => {
    fetch(
      `${getApiUrl()}/Student/GetGeneralEventInstallments/${studentId}/${id}`
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const calculateTotal = () => {
    ...
    return unpaidInstallments
      ...
      .reduce(
        (total, installment) => total + (installment.installment ?? 0),
        0
      );
  };
  ...
  return (
    <>
      ...
      <Container className="mt-4">
          ...
          <tbody>
            {installments.map((installment, index) => (
              <tr key={index}>
                ...
                <td>
                  {installment.payDate
                    ? new Date(installment.payDate).toLocaleDateString()
                    : 'N/A'}
                </td>
        ...
        <p className="mt-3">
          <b>Total Selecionado:</b> R$ {calculateTotal().toFixed(2)}
        </p>
        ...
        <Button className="mt-3" onClick={handleGerarPDF} variant="info">
          Gerar recibo
        </Button>
      </Container>
