...
function GeneralEventsPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/GeneralEvent/AddGeneralEvent`,
        {
          method: 'POST',
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/GeneralEvent/DeleteGeneralEvent/${eventIdToDelete}`,
        {
          method: 'DELETE',
        }
      );
              ...
              {generalEvents.map((event) => (
                <tr key={event.id}>
                  ...
                  <td>
                    <Button
                      variant="info"
                      onClick={() => navigate(`/ViewGeneralEvent/${event.id}`)}
                    >
                      Acessar
                    </Button>{' '}
                    <Button
                      variant="warning"
                      onClick={() => navigate(`/EditGeneralEvent/${event.id}`)}
                    >
                      Editar
                    </Button>{' '}
                    <Button
