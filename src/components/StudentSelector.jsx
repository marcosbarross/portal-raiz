...
const StudentSelector = ({ onStudentSelect }) => {
  ...
  useEffect(() => {
    axios.get(`${getApiUrl()}/Group/GetGroupsDetails`)
      .then(response => {
        const groupsData = response.data || [];
        setGroups(groupsData);

        const uniqueLevels = [
          ...new Map(groupsData.map(group => [group.level, { id: group.level, name: group.level }])).values()
        ];
  ...
  const onGroupChange = (e) => {
    ...
    if (groupId) {
      axios.get(`${getApiUrl()}/Student/GetStudentsByGroup/${groupId}`)
        .then(response => {
  ...
  return (
    <>
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        ...
        <Modal.Body>
          O estudante {students.find(student => student.registration === parseInt(selectedStudent))?.name} foi selecionado com sucesso!
        </Modal.Body>
        <Modal.Footer>
