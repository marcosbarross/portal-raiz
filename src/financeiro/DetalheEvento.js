...
function DetalheEvento() {
  ...
  const fetchEventDetails = async () => {
    try {
      const response = await axios.get(
        `${getApiUrl()}/event/GetEventDetails/${id}`
      );
  ...
  const fetchStudentsByGroup = async (groupId) => {
    try {
      const response = await axios.get(
        `${getApiUrl()}/group/GetGroupDetail/${groupId}`
      );
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      ...
      alert(
        'Erro ao adicionar aluno. Verifique se o aluno já está registrado no evento.'
      );
    }
  };
  ...
  const handlePagarParcelas = async (studentId) => {
    ...
    try {
      await axios.post(
        `${getApiUrl()}/student/PagarParcelas`,
        selectedParcelas
      );
  ...
  const handleGerarPDF = (studentName, parcelasPagas) => {
    ...
    pdf.text('Rua Francisco do Rego Moraes Barros', docWidth / 2, 13, {
      align: 'center',
    });
    pdf.text('Engenho Maranguape - Paulista - PE', docWidth / 2, 16, {
      align: 'center',
    });
  ...
  const fetchParcelas = async (studentId) => {
    ...
    try {
      const response = await axios.get(
        `${getApiUrl()}/student/GetStudentParcelas/${studentId}`
      );
  ...
  const handleCheckboxChange = (studentId, parcelaIndex) => {
    ...
    if (Array.isArray(updatedParcelas[studentId])) {
      ...
      const allPreviousPaid = updatedParcelas[studentId]
        .slice(0, parcelaIndex)
        .every((parcela) => parcela.Paid || parcela.paid);
      if (allPreviousPaid) {
      ...
      <Container className="mt-4">
        {event ? (
          <>
            ...
            <p>
              Pagamento: R${event.TotalPrice} reais em {event.Installments}{' '}
              parcelas de R$ {event.TotalPrice / event.Installments}
            </p>
            ...
            <Accordion defaultActiveKey="0">
              {event.Students.map((student) => (
                <Accordion.Item
                  eventKey={student.Registration}
                  key={student.Registration}
                >
                  <Accordion.Header
                    onClick={() => handleToggleAccordion(student.Registration)}
                  >
                    {`${student.Name}, Responsável: ${student.Responsible}`}
                  </Accordion.Header>
                  <Accordion.Body>
                    {loadingParcelas[student.Registration] ? (
                      ...
                      <Table striped bordered hover>
                        ...
                        <tbody>
                          {parcelas[student.Registration]?.map(
                            (parcela, index) => (
                              <tr key={index}>
