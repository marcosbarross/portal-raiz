...
function TurmaDetalhe() {
  ...
  const confirmDelete = () => {
    axios
      .delete(
        `${getApiUrl()}/group/RemoveStudentFromGroup/${studentIdToDelete}`
      )
      .then(() => {
        loadGroupData(id);
        setShowModal(false);
  ...
  const loadGroupData = (groupId) => {
    axios
      .get(`${getApiUrl()}/group/GetGroupDetail/${groupId}`)
      .then((response) => {
        if (response.data) {
          setGroupData({
            ...response.data,
  ...
  const filteredStudents = groupData.students.filter(
    (student) =>
  ...
  return (
    <>
      ...
      <Container>
        <h1>
          {groupData.name || 'Turma não informada'},{' '}
          {groupData.shift || 'Turno não informado'}
        </h1>
              ...
              {filteredStudents.map((student, index) => (
                <tr key={student.registration || index}>
                  ...
                  <td>
                    <Button
                      variant="danger"
                      onClick={() => handleDeleteStudent(student.registration)}
                    >
                      Remover
                    </Button>
                  </td>
